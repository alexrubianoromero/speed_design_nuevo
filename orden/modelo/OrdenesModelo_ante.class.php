...
Class OrdenesModelo extends Conexion
{
    ...
    public function verificarDuplicidadOrden($request)
    {
        $sql = "select * from ordenes 
                where placa= '".$request['placa']."'      
                and tipo_medida_kms_millas_horas = '".$request['tipo_medida']."'      
                and kilometraje = '".$request['kilometraje']."'    
                and mecanico = '".$request['mecanico']."'    
                and observaciones = '".$request['descripcion']."'    
                "; 
