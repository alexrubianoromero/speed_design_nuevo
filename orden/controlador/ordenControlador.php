...
class ordenControlador 
{
    ...
    public function grabarOrden($conexion,$datos){
        //verifica si ya existe una orden con esta misma informacion 
        $filas =  $this->modeloOrden->verificarDuplicidadOrden($datos); 
        if($filas == 0)
        {
    ...
    public function grabarNuevoItemOrden($request){
        ...
        $ultimoIdMovimiento = $this->movimientosModelo->registerMovNew($data,$request);

        // die('ultimo Id Mov '.$ultimoIdMovimiento); 
        ...
        $this->codigosModelo->saveMoreLessInventNew($request,$ultimoIdMovimiento);
